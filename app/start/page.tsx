...
function ExternalLink({ href, children, secondary = false }: { href: string; children: React.ReactNode; secondary?: boolean }) {
  return <a className={`start-button${secondary ? " start-button-secondary" : ""}`} href={href} target="_blank" rel="noreferrer">{children}<Arrow /></a>;
}
  ...
  return (
    <main className="start-page" id="top">
      ...
      <section className="start-section start-shell" id="self"><article className="start-route-card">
        ...
        <ExternalLink href={routes.starterBundle}>Переглянути комплект</ExternalLink>
      </article></section>
      ...
      <section className="start-section start-section-warm" id="courses"><article className="start-route-card start-shell">
        ...
        <div className="start-options">
          <div><strong>«Здорова тарілка»</strong><span>Як збирати прийоми їжі без дієтичного меню.</span><b className="start-price">490 грн</b><ExternalLink href={routes.healthyPlate}>Перейти до курсу</ExternalLink></div>
          <div><strong>«Більше руху»</strong><span>Як додати активність без спортзалу й героїчних планів.</span><b className="start-price">490 грн</b><ExternalLink href={routes.movementCourse}>Перейти до курсу</ExternalLink></div>
        </div>
      </article></section>
      ...
      <section className="start-section start-shell" id="support"><article className="start-route-card start-route-featured">
        ...
        <div className="start-actions"><ExternalLink href={routes.supportPage}>Як проходить супровід</ExternalLink><ExternalLink href={routes.supportBot} secondary>Залишити заявку</ExternalLink></div>
      </article></section>
